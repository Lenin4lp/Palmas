...
function AliquotModify() {
  ...
  const aliquot = aliquotData.data.monthlyFee;
  ...
  const modifyAliquot = async (id, aliquot) => {
    try {
      const res = await updateMonthlyFee(id, aliquot);
      if (res.status === 200) {
        toast.success("Alicuota modificada con éxito");
        setTimeout(() => {
  ...
  const removeAliquot = async (id) => {
    try {
      const res = await deleteMonthlyFee(id);
      if (res.status === 204) {
        toast.success("Alicuota eliminada con éxito");
        setTimeout(() => {
          window.location.href = `/alicuotas`;
        }, 2000);
      }
    ...
    <div className=" md:pl-[70px] h-fit min-h-screen w-screen md:py-0 pb-[90px] flex justify-center items-start">
        ...
        <div className=" block m-3">
          ...
          <div className=" my-3">
            <h1 className=" text-center text-white text-base font-medium">
              ¿Estás seguro de modificar la alicuota?
            </h1>
          </div>
      ...
      <Modal open={open2} onClose={() => setOpen2(false)}>
        <div className=" block m-3">
          ...
          <div className=" my-3">
            <h1 className=" text-center text-white text-base font-medium">
              ¿Estás seguro de eliminar la alicuota?
            </h1>
          </div>
          <div className=" flex justify-center items-center">
            <div className=" my-2 grid grid-cols-2">
              <div className=" mx-4">
                <button
                  onClick={() => removeAliquot(aliquot.monthlyFee_id)}
                  className=" p-2 active:transform active:scale-90 border border-white bg-[#384c85]  rounded-lg hover:bg-[#146898] text-white hover:text-white text-[12px] md:text-sm lg:text-base duration-500"
                >
                  Aceptar
                </button>
        ...
        {id != 1 && (
          <div className=" m-9 flex justify-center items-center">
            <button
              ...
              className=" hover:bg-[#a32b2b] bg-[#8b1e1e] duration-300 transition border-[1px] border-[#8f0e2a] p-2 text-white rounded-lg"
            >
              Eliminar
            </button>
          </div>
